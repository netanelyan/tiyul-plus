...
/**
 * עמוד הפרימיום: השוואת תוכניות + שדרוג. שלושת המצבים הכנים:
 ...
 *
 * **מצב נכון לעכשיו, ולמה זה כתוב כאן ולא רק ב-plans.ts.** `/api/billing/checkout`
 * הוא Stripe, ואינו מוגדר - אף מנוי מעולם לא חויב דרכו. הספק הפעיל
 * היחיד באתר הוא PayPal (`server/paypal.ts`), שכבר מריץ תשלום אמיתי
 * עבור "בדיקה לפני הנסיעה". שורת האמון למטה נכתבה לפי זה - **לא לפי
 * הקוד שהכפתור בפועל קורא לו** - כי הוא עדיין מחזיר "בקרוב" בכל מקרה.
 * מי שמפעיל את ההרשמה לפרימיום צריך להחליט קודם דרך איזה משני
 * הנתיבים זה יקרה, ולעדכן את שניהם ביחד.
 */
  ...
  return (
    <div className="rise-in mx-auto max-w-3xl">
      ...
      <p className="mt-6 text-center text-xs leading-relaxed text-night/45">
        התשלום מאובטח דרך PayPal - אנחנו לא רואים ולא שומרים פרטי אשראי. אפשר לבטל בכל רגע,
