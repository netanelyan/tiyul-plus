 ...
 * אחרי ארבע הודעות.
 *
 * מכאן: תקרה אישית $3.00, והיא **נגזרת מהיום כולו** (12%), ולכן היום
 * הוא $25. זו תקרה, לא הוצאה: בתנועה של היום ההוצאה בפועל היא דולרים
 * בודדים. $25 ליום הם $750 לחודש במקרה הקיצוני ביותר, ושדה טקסט אחד
 * ב-/admin משנה את זה בלי דיפלוי.
 */
 ...
 *
 * הועלה מ-30% ל-40% ($10) כי הדרישה השתנתה: מבקר אנונימי צריך להשלים
 * **סשן תכנון מלא** ולצאת משוכנע, ובעלות שנמדדה זה כ-$2.1. ב-40% נכנסים
 * ארבעה כאלה ביום, ובפועל יותר - סשן על מטמון חם עולה כ-$1.6.
 * המחוברים עדיין מוגנים: הרצפה שלהם היא 60% מהיום, $15.
 */
export const ANON_SHARE = 0.4;

/**
 * החלק המרבי של היום שזהות **אחת** יכולה להוציא. 12% מ-$25 = **$3.00**.
 *
 * המספר נגזר מהסשן ולא מהתקרה: סשן אנונימי מלא הוא כ-$2.1 טיפוסי וכ-$2.7
 * בגרוע, אז $3 הוא "בנוחות מעל" ולא "בדיוק". צריך שמונה מנצלים כדי
 * למצות יום שלם, ובפועל יותר - מכסת ההודעות היומית חוסמת הרבה קודם.
 */
export const CALLER_SHARE = 0.12;

/**
 * **התקרה האישית נגזרת מהיום כולו, גם לאנונימי** - זו הייתה ההוראה
 * המפורשת, והיא תיקון של באג ולא כוונון.
 *
 * קודם היה כאן 15% **מהארנק האנונימי**, כלומר $0.225 - פחות מקריאה קרה
 * אחת שנמדדה ב-$0.447. מבקר אנונימי נחסם אחרי הודעה או שתיים, וזה בדיוק
 * מה שקרה. עכשיו זה אותו $3.00 שמקבל מחובר.
 *
 * המשמעות: אנונימי אחד יכול לקחת עד 30% מהארנק האנונימי. זו החלפה
 * מודעת - "מבקר אחד שמשלים סשן" עדיף על "ארבעה מבקרים שכולם נחסמים
 * באמצע", וכל מי שבאמת מנצל נתקל קודם במכסת ההודעות ובמגבלת הפרץ.
 */
export const ANON_CALLER_SHARE = CALLER_SHARE;
...
export async function budgetFor(identity: string): Promise<BudgetState> {
  ...
  */
  ...
  /*
    **מהיום כולו, לא מהארנק.** `ANON_SHARE * ANON_CALLER_SHARE` נתן
    $0.225 - פחות מקריאה קרה אחת. זו הייתה החסימה אחרי ארבע הודעות.
  */
  const callerBudget = budget * (anon ? ANON_CALLER_SHARE : CALLER_SHARE);
